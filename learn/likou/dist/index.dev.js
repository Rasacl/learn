"use strict";

/**
 * 给你一个整数数组 nums ，和一个表示限制的整数 limit，请你返回最长连续子数组的长度，该子数组中的任意两个元素之间的绝对差必须小于或者等于 limit 。

   如果不存在满足条件的子数组，则返回 0 。
   输入：nums = [8,2,4,7], limit = 4
输出：2 
解释：所有子数组如下：
[8] 最大绝对差 |8-8| = 0 <= 4.
[8,2] 最大绝对差 |8-2| = 6 > 4. 
[8,2,4] 最大绝对差 |8-2| = 6 > 4.
[8,2,4,7] 最大绝对差 |8-2| = 6 > 4.
[2] 最大绝对差 |2-2| = 0 <= 4.
[2,4] 最大绝对差 |2-4| = 2 <= 4.
[2,4,7] 最大绝对差 |2-7| = 5 > 4.
[4] 最大绝对差 |4-4| = 0 <= 4.
[4,7] 最大绝对差 |4-7| = 3 <= 4.
[7] 最大绝对差 |7-7| = 0 <= 4. 
因此，满足题意的最长子数组的长度为 2 。
 */

/**
 * @param {number[]} nums
 * @param {number} limit
 * @return {number}
 */
var longestSubarray = function longestSubarray(nums, limit) {
  // 维护2个单调队列
  var queueMax = [];
  var queueMin = [];
  var n = nums.length;
  var left = 0,
      right = 0;
  var res = 0;

  while (right < n) {
    // 维护max的队列
    while (queueMax.length && queueMax[queueMax.length - 1] < nums[right]) {
      queueMax.pop();
    } // 维护min队列


    while (queueMin.length && queueMin[queueMin.length - 1] > nums[right]) {
      queueMin.pop();
    }

    queueMax.push(nums[right]);
    queueMin.push(nums[right]);
    right++; // 判断条件, 当不成立的时候，左边内收

    if (queueMax.length && queueMin.length && queueMax[0] - queueMin[0] > limit) {
      // 从左边开始排除最小值
      if (nums[left] === queueMin[0]) {
        queueMin.shift();
      } // 从左边开始排除最大值


      if (nums[left] === queueMax[0]) {
        queueMax.shift();
      } // 内收左边


      left++;
    } else {
      // 求最长的值
      res = Math.max(res, right - left);
    }
  }

  return res;
}; // longestSubarray([4,2,2,2,4,4,2,2],0)

/**
 * @param {number} x
 * @return {boolean}
 */


var isPalindrome = function isPalindrome(x) {
  var y = Number(String(x).split('').reverse().join(''));
  return y == x;
};

console.log(isPalindrome(-121));